...
describe('78: `Promise` API overview', function() {
  ...
  describe('`Promise.all()`', () => {
    it('`Promise.all([p1, p2])` resolves when all promises resolve', () => {
      ...
      return Promise.all([resolvingPromise, resolvingPromise, resolvingPromise])
    });
  ...
  describe('`Promise.race()`', () => {
    it('`Promise.race([p1, p2])` resolves when one of the promises resolves', () => {
      ...
      return Promise.race([resolvingPromise, rejectingPromise])
    });
